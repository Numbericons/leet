...
var romanToInt = function (s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    const negKeys = Object.keys(posNegs);
    const nextIsBigger = posNegs[s[i]][0] === (s[i + 1]) || posNegs[s[i]][1] === (s[i + 1]);
    const negVal = negKeys.includes(s[i]) && nextIsBigger //the current char is negative when the next char is one of 2 values
